...
public class TictacoeGame {
	...
	private static boolean isRightDiagonal(char[][] matrix, int row, int col, char symb) {
		if(isOnRightDiagonal(row, col, matrix.length -1)) {
	...
	private static boolean isOnRightDiagonal(int row, int col, int maxIndex) {
		
		return row + col == maxIndex;
	...
	private static boolean isFullColumn(char[][] matrix, int col, char symb) {
		boolean res = true;
		for(int i = 0; i < matrix.length; i++) {
			if(matrix[i][col] != symb) {
				res = false;
				break;
			}
		}
